...
static void init() {
    ...
    prog = compile_gl_program("opengl-vector-vert.glsl", "opengl-vector-frag.glsl");
    ...
    GLCHECK(glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex) * 2 * BATCH_COUNT, nullptr, GL_STREAM_DRAW));
...
static void flush() {
    GLCHECK(glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(Vertex) * batch.head * 2, batch.batch));
    GLCHECK(glDrawArrays(GL_LINES, 0, batch.head * 2));
    ++draws;
    batch.head = 0;
}
...
REGISTER_DEMO("Vector", init, frame, ui);
